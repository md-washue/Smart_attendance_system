...
A local facial recognition attendance system combining a React Native mobile interface with a high-performance FastAPI Python backend. This application replaces manual roll calls by capturing student faces via a mobile device and processing them against an LBPH face recognizer to automatically log attendance into a SQLite database.
...
* **Automated Data Logging:** Securely records timestamped attendance matches directly into a local SQLite database using parameterized queries.
...
* **Native Network Handling:** Utilizes `expo-file-system` to bypass React Native's Android `FormData` limitations, explicitly enabling local cleartext (`http://`) traffic for seamless local network communication.
